...
# Terraform Associate (004) — Question Bank Iter 10 Batch 2

**Iteration**: 10
**Iteration Style**: Exam-style scenarios — multi-sentence real-world context questions
**Batch**: 2
**Objective**: 2 — Terraform Fundamentals (Providers, Plugin Model, State)
**Generated**: 2026-04-26
**Total Questions**: 13
...
**Explanation**:
Providers embed a schema that defines valid arguments, required vs optional status, and allowed types for every resource and data source they expose. During `terraform validate` and `terraform plan`, Terraform Core checks the configuration against this provider-supplied schema locally — before making any API call. This schema validation is one of the five core responsibilities of a Terraform provider plugin.
...
**Explanation**:
When multiple configurations of the same provider type exist (distinguished by aliases), each resource targeting a non-default configuration must explicitly declare `provider = aws.<alias>`. Resources that omit the `provider` meta-argument automatically use the default provider (the `provider "aws"` block without an `alias`). There is no implicit round-robin distribution or automatic alias matching from `for_each`.
...
**Explanation**:
`sensitive = true` is a display-only control — it causes Terraform to render `(sensitive value)` in CLI output rather than showing the actual value. The underlying data is written to `terraform.tfstate` in plaintext regardless of this setting. Protecting secrets at rest requires encrypting the storage backend (e.g., S3 server-side encryption, Azure Blob encryption) or, better, not placing secrets in state at all by using dynamic retrieval from a tool like Vault. Options B and D describe non-existent Terraform features.
...
**Explanation**:
State locking is a deliberate safety mechanism in remote backends that support it (S3 + DynamoDB is one such backend). When one `apply` acquires the lock, all concurrent operations attempting to modify the same state are blocked until the lock is released. This prevents race conditions that could corrupt the state file or produce conflicting resource changes. Engineer B's error is correct and expected behaviour — it is not a sign of infrastructure damage.
...
**Explanation**:
`terraform state mv <old-address> <new-address>` updates the state file to associate the existing cloud resource with the new configuration address. After this command, `terraform plan` will report "0 to add, 0 to change, 0 to destroy" because the resource in state now matches the new label in the configuration. Option A (`prevent_destroy`) blocks deletion but does not resolve the plan's destroy+create intent. Option D (`import`) could create a duplicate state entry if the old address is not removed first.
...
**Explanation**:
`>= 4.0` sets a floor but no ceiling, allowing `terraform init` on a fresh workstation to install the latest available version — including major versions with breaking changes. The pessimistic constraint `~> 5.0` means `>= 5.0 and < 6.0`, confining all installs to major version 5. The new engineer's machine had no existing lock file to constrain the selection, so it received the latest matching version (v6.0.0). Tightening the constraint and committing the lock file together ensures all team members install the same provider version.
...
**Explanation**:
`.tfstate.backup` is a copy of the state file from immediately before the last `apply` began — it has zero knowledge of the four resources created during the partial apply. Restoring it leaves Terraform unaware of those resources, risking orphaned cloud objects. On the next apply, Terraform would plan to create all five resources again; since four already exist, the apply would likely fail with resource conflict errors or create duplicates. The correct recovery is to keep the current `terraform.tfstate` (which tracks the four created resources) and simply re-run `apply` to create the remaining EC2 instance.
...
**Explanation**:
`terraform state list` outputs all resource addresses currently tracked in the active state file, one per line, in the format `resource_type.resource_name` (or `module.<name>.<type>.<name>` for module resources). It provides a concise inventory without showing full attribute details. `terraform show` outputs the entire state with all attributes (much more verbose). `terraform state show <address>` shows attributes for a single specified resource. `terraform output` shows declared output values, not resource addresses.
...
**Explanation**:
`terraform init -migrate-state` instructs Terraform to automatically copy existing state from the current backend to the newly declared backend without requiring interactive confirmation — making it suitable for CI/CD pipelines. `-reconfigure` re-initializes the backend without copying the existing state (the old state is abandoned from Terraform's perspective). `-backend-config` passes partial backend configuration values. `-upgrade` upgrades provider and module versions to the latest matching their constraints.
...
**Explanation**:
**A** is correct — Partner providers are written and actively maintained by HashiCorp's technology partners (third-party companies with a formal relationship), not by HashiCorp itself. **C** is correct — HashiCorp reviews and approves Partner providers for the registry badge, giving them a verified status above Community providers while below the Official tier. The Partner badge satisfies both security policy requirements the team cited. **B** is false — the review rigor differs between Official and Partner tiers. **D** accurately describes the Community tier, not the Partner tier.
...
**Explanation**:
The pessimistic constraint `~> 1.5.0` (three-part version) restricts updates to the rightmost component only — patch level. It translates to `>= 1.5.0 and < 1.6.0`. Terraform v1.8.0 is in the 1.8.x series, outside the `< 1.6.0` upper boundary, so it fails the `required_version` check. This is distinct from `~> 1.5` (two-part), which allows minor updates up to (but not including) v2.0.0. To use Terraform v1.8.0 with this module, the module's `required_version` must be updated to a constraint that includes v1.8.0 (e.g., `~> 1.8` or `>= 1.5`).
...
**Explanation**:
Terraform's plugin architecture separates each provider into its own independent OS process. Terraform Core communicates with each provider process over a local gRPC connection. Because each provider runs in a separate process, a panic or crash in one provider's binary terminates only that process — Terraform Core detects the failed gRPC call, records an error for the affected resource, and continues its execution graph for resources managed by other providers. This process-level isolation is a fundamental and deliberate design property of Terraform's plugin model.
...
**Explanation**:
`terraform state rm <address>` removes the specified resource from Terraform's state file, causing Terraform to stop tracking it. Critically, it makes no API calls — the RDS instance is completely unaffected in AWS. After this command, the database operations team can run `terraform import aws_db_instance.postgres <db-identifier>` in their workspace to take over management. Option D is incorrect and dangerous — removing a resource block and running `apply` causes Terraform to destroy the actual cloud resource, because Terraform's declarative model interprets "not in config" as "should not exist."
