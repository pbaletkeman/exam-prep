...
|----|-----------|-----------:|--------|------------|
| 1 | C | N/A | What Terraform reports when infrastructure already matches the configuration | Easy |
| 2 | B | N/A | Terraform's response when a resource is manually deleted from the cloud | Easy |
| 3 | C | N/A | What happens when a declarative config reduces a resource count | Medium |
| 4 | B | N/A | What terraform plan shows when count is increased | Medium |
| 5 | B | N/A | What happens when a manual change is made outside Terraform then apply runs | Medium |
| 6 | A, B | N/A | Behavioural differences between Terraform (declarative) and Ansible (imperative) on repeated runs | Medium |
| 7 | B | N/A | Audit trail behaviour when infrastructure is changed outside of code | Medium |
| 8 | C | N/A | What happens when a config declares resources from multiple cloud providers | Medium |
| 9 | B | N/A | What apply produces when recreating a lost environment from code and state backup | Medium |
| 10 | A, B | N/A | Behavioural prediction when count is set to zero and apply runs | Medium |
| 11 | B | N/A | Reproducibility across environments using identical IaC configuration | Hard |
| 12 | B | N/A | Behavioural change when a team adopts IaC after managing infrastructure manually | Hard |
