...
|----|-----------|-----------:|--------|------------|
| 1 | B | N/A | Idempotency — "No changes" as expected and correct behaviour | Easy |
| 2 | A | N/A | IaC as living documentation of infrastructure | Easy |
| 3 | C | N/A | Terraform enforces desired state — manual out-of-band changes are reverted on next apply | Medium |
| 4 | B | N/A | CloudFormation is AWS-only; Terraform provides multi-cloud support | Medium |
| 5 | B | N/A | Disaster recovery using IaC — restoring infrastructure from configuration and state backup | Medium |
| 6 | A | N/A | Idempotency contrast — imperative scripting vs declarative Terraform | Medium |
| 7 | A, C | N/A | Infrastructure drift and repeatability — restoring environment parity via Terraform apply | Medium |
| 8 | B | N/A | IaC audit trail — Git commit history and pull request records as compliance evidence | Medium |
| 9 | A, C | N/A | Provisioning tools vs configuration management tools — Terraform and Ansible in a full-stack workflow | Medium |
| 10 | B | N/A | Desired state vs current state — Terraform computes the minimal change set | Medium |
| 11 | B | N/A | Terraform's declarative model — engineers declare desired end state, not execution steps | Hard |
| 12 | A, C | N/A | Drift detection without making changes — `terraform plan` and `terraform plan -refresh-only` | Hard |
